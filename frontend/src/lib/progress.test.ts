...
describe("progress unlock policy helpers", () => {
  ...
  it("enforces sequential unlock inside a unit", () => {
    ...
    expect(isLessonUnlocked(TRACK, UNIT_A, LESSON_A1, keys, "unit-sequential")).toBe(true);
    expect(isLessonUnlocked(TRACK, UNIT_A, LESSON_A2, keys, "unit-sequential")).toBe(false);
  });

  it("finds and routes to first blocking prior lesson", () => {
    const keys = new Set<string>();
    const blocker = getFirstBlockedLessonBeforeTarget(TRACK, UNIT_A, LESSON_A2, keys, "unit-sequential");
    expect(blocker?.lessonSlug).toBe(LESSON_A1);
    expect(getNextEligibleLessonPath(TRACK, UNIT_A, LESSON_A2, keys, "unit-sequential")).toBe(
  ...
  it("supports track-wide policy abstraction", () => {
    ...
    expect(isLessonUnlocked(TRACK, UNIT_B, LESSON_B1, keys, "track-sequential")).toBe(true);
    keys.delete(lessonKey(TRACK, UNIT_A, LESSON_A2));
    expect(isLessonUnlocked(TRACK, UNIT_B, LESSON_B1, keys, "track-sequential")).toBe(false);
  });

  it("returns the first incomplete lesson in global curriculum order", () => {
    const keys = new Set<string>();
    const first = getFirstIncompleteLessonPointer(keys);
  ...
  it("skips completed lessons when resolving resume pointer", () => {
    ...
    const next = getFirstIncompleteLessonPointer(keys);
